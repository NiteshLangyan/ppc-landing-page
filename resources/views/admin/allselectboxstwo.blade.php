      ...
      <div class="card">
        ...
        <div class="card-datatable table-responsive">
          <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody>
                @if(!$selectboxstwo->isEmpty())
                 @foreach($selectboxstwo as $listing)
                <tr>
                <td>{{ $listing->sort }} </td> 
                <td>{{ $listing->heading }} </td> 
                <td> <a href="{{ url('') }}/adm/selectboxstwo/{{ $listing->id }}">
                    <button type="button" class="btn btn-warning"><span class="tf-icons bx bx-pencil"></span> Edit</button>
                 </a>   </td>
                <td>  <a href="{{ url('') }}/adm/selectboxstwodel/{{ $listing->id }}" onclick="return confirm('Are you Sure?')">
