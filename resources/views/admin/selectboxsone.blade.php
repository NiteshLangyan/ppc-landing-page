      ...
      <div class="card-body">
        <form method="POST" action="{{ $url }}" class="row g-3" enctype="multipart/form-data">
          ...
          <div class="col-md-4">
            ...
            <input class="form-control" type="text"  name="sort" required  @if(!empty($selectboxsone->sort)) value="{{ $selectboxsone->sort }}" @endif >
            <small style="font-size: 70%; color: #ec1b1b; text-transform: capitalize;"> @error('sort') {{$message}} @enderror </small>
        </div>

          <div class="col-md-4">
            <label class="form-label" >Text</label>
            <input class="form-control" type="text" name="heading" required  @if(!empty($selectboxsone->heading)) value="{{ $selectboxsone->heading }}" @endif >
