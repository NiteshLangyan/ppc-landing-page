      ...
      <div class="card">
        ...
        <div class="card-datatable table-responsive">
          <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody>
                @if(!$selectboxsthree->isEmpty())
                 @foreach($selectboxsthree as $listing)
                <tr>
                <td>{{ $listing->sort }} </td> 
                <td>{{ $listing->heading }} </td> 
                <td> <a href="{{ url('') }}/adm/selectboxsthree/{{ $listing->id }}">
                    <button type="button" class="btn btn-warning"><span class="tf-icons bx bx-pencil"></span> Edit</button>
                 </a>   </td>
                <td>  <a href="{{ url('') }}/adm/selectboxsthreedel/{{ $listing->id }}" onclick="return confirm('Are you Sure?')">
